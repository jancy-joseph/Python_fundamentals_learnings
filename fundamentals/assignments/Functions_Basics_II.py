...
""" def CreateNewList(num):
    newlist=[]
    for i in range(5,-1,-1):
        newlist.append(i)
    return newlist
mynewlist = CreateNewList(5)
print(mynewlist) """
...
""" def first_plus_length(myarray):
    if(len(myarray)>0):
        return(myarray[0]+len(myarray))
    else:
        return len(myarray)# basically zero

arr1=[3,4,5,6]
result = first_plus_length(arr1)
print(result) """
...
def length_and_value(mysize,myvalue):
    newlist =[]
    for i in range(mysize):
        newlist.append(myvalue)
    return newlist
mynewlist = length_and_value(4,7)
print(mynewlist)
mynewlist = length_and_value(6,2)
print(mynewlist)
